...
int insert_at( int fd, off_t offset, char *buf, int n ) {
    ...
    int bytesToMove = fileSize - offset;
    printf("bytesToMove %d\n", bytesToMove);
    if (bytesToMove > 0) {
        char *temp = (char*) malloc( bytesToMove * sizeof(char) );
        lseek( fd , 0, SEEK_SET );
        int bytesRead = read( fd, temp, bytesToMove );
        if(bytesRead == -1) {
            printf("Error while reading bytes\n");
            return -1;
        }
        printf("bytesRead %d\n", bytesRead);
        int newOffset = replace_at( fd, offset, buf, n );
        return replace_at( fd, newOffset, temp, bytesRead );
    } else return replace_at( fd, offset, buf, n );
}
...
int main(int argc, char **argv) {
    ...
    int destFd = open(destFile, O_RDWR | O_CREAT, S_IRUSR | S_IWUSR);
    ...
    insert_at( destFd, offset, buffer, numBytes );
